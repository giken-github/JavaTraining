...
本システムの画面レイアウトは、次の通りとする。なお、背景色は領域の区切りを表す。

| 領域         | 内容                                                        |
|--------------|------------------------------------------------------------|
| ヘッダー     | システムロゴ、機能メニュー、ログイン名を表示する。         |
| 機能メニュー | クリックすると各機能へ移動するリンクを列挙して表示する。   |
| ログアウト   | クリックするとログアウトするボタンを表示する。             |
| タイトル     | 機能のタイトルを表示する。                                 |
| 機能のボディ | 各機能の内容を表示する。                                   |
...
- トップ → トップページ(/) へ遷移する
- 加入者管理 → 加入者検索条件画面(`/member/search`)へ遷移する。
...
`/logout`にPOSTで遷移しログアウト処理をする。

!!! info

    `/logout`はSpring Security 機能から提供されているURLです。
...
入力された「メールアドレス」と「氏名」で加入者情報を部分検索し、その結果を「**加入者検索結果一覧画面**」に表示する。

なお、「メールアドレス」と「氏名」両方指定された場合は、両者の条件を満たす加入者情報を抽出する。したがって、両者とも空の場合は、すべての加入者情報が出力される。
...
|     | (項目) | 　加入者ID       | リンク     | ―          | クリックすると加入者編集画面を表示。     |
|     |        | 　メールアドレス | テキスト   | ―          |                                          |
|     |        | 　氏名           | テキスト   | ―          |                                          |
|     |        | 　住所           | テキスト   | ―          |                                          |
|     |        | 　加入日         | テキスト   | YYYY/MM/DD |                                          |
...
加入者IDに対応する加入者情報の**加入者編集画面**を表示する。
...
- 追加モード … 加入者情報と適用料金情報を新たに追加するモード。入力可能な項目はすべて空欄にして表示する。

- 編集モード … 指定された加入者IDを持つ加入者情報と適用料金情報を編集するモード。加入者IDに該当する加入者情報と適用料金情報を取得して、入力可能な項目にセットして表示する。
...
※ 入力内容に誤りがあり、加入者情報の変更に失敗した場合の表示は追加モードと同様、エラーのある項目の入力欄のそばにエラー内容を表示する。
...
入力された「料金名」を条件に料金情報から部分検索し、その結果を「**料金検索結果一覧画面**」に表示する。
...
| ①  | back   | 戻る         | ボタン     | ―          |                |
| ②  | result | 検索結果一覧 | テーブル   | ―          | 行数制限なし   |
|     | (項目) | 　料金ID     | リンク     | ―          | クリックすると料金情報編集画面を表示。 |
|     |        | 　料金名     | テキスト   | ―          |                |
|     |        | 　月額料金   | テキスト   | ―          |                |
...
- 追加モード … 料金情報を新たに追加するモード。入力可能な項目はすべて空欄にして表示する。
- 編集モード … 指定された料金IDを持つ料金情報を編集するモード。料金IDに該当する料金情報を取得して、入力項目に設定して表示する。

### 構成

![](media/screen-KA020V020-charge-edit.png)
...
入力された情報を料金情報に追加する。正常に完了したら、登録した情報を各入力項目に設定した「**料金情報編集画面**」を編集モードで表示する。
...
入力された情報で料金情報を更新する。正常に完了したら、登録した情報を各入力項目に設定した「**料金情報編集画面**」を編集モードで表示する。

また、正常に料金情報が変更できた場合はタイトル下部に「保存しました」とメッセージを表示する。

※ 入力内容にエラーがある場合の表示は追加モード時と同様、エラーのある項目の入力欄のそばにエラー内容を表示する。
